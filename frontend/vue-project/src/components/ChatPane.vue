...
<template>
  <main class="flex-1 flex flex-col h-full bg-slate-950 min-w-0 relative">
    ...
    <header class="h-16 border-b border-slate-800 px-6 flex items-center justify-between bg-slate-900/50 backdrop-blur shrink-0">
      ...
      <div class="flex items-center gap-3">
        <button
          v-if="!store.isArtifactPanelOpen && store.activeArtifact"
          @click="store.toggleArtifactPanel"
          class="px-2.5 py-1.5 rounded-lg bg-indigo-950 text-indigo-300 border border-indigo-800 hover:bg-indigo-900 text-xs font-medium flex items-center gap-1.5 transition-colors"
